...
from helper.helper_func import encode, get_message_id, get_redirect_link, generate_qr   # <-- added generate_qr
import asyncio
...
async def batch(client: Client, message: Message):
    ...
    if client.qr_enabled:
        qr_image = await generate_qr(direct_link)
    
    # Send QR code first (no quote possible)
        await second_message.reply_photo(
            photo=qr_image,
            caption="🕸️ ꜱᴄᴀɴ Qʀ ᴄᴏᴅᴇ:",
            reply_markup=reply_markup,
            parse_mode=ParseMode.HTML
        )
    
    # Then send quoted text message
        await second_message.reply_text(
            response_text,
            quote=True,  # ✅ This works here
            reply_markup=reply_markup,
            parse_mode=ParseMode.HTML,
...
async def link_generator(client: Client, message: Message):
    ...
    # Send QR code first (no quote possible)
        await channel_message.reply_photo(
            photo=qr_image,
            caption="🕸️ ꜱᴄᴀɴ Qʀ ᴄᴏᴅᴇ:",
            reply_markup=reply_markup,
            parse_mode=ParseMode.HTML
